...
until (curl --silent http://127.0.0.1:4566/health | grep "\"s3\": \"\(running\|available\)\"" > /dev/null);do 
    sleep 5
done
...
aws --endpoint-url=http://127.0.0.1:4566 s3api create-bucket --bucket fragments
...
aws --endpoint-url=http://127.0.0.1:8000 dynamodb wait table-exists --table-name fragments
